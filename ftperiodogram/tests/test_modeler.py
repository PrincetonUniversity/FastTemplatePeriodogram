"""Tests of high-level methods for modeler class"""
import numpy as np
...
def data_from_template(template, parameters, N=ndata0, T=2,
    ...
    rand = np.random.RandomState(rseed)
    model = TemplateModel(template, parameters=parameters,
                          frequency=(1./period))
...
def shift_template(template, tau):
    def tmpl(phase, tau=tau):
        return template(phase - tau)
    return tmpl
...
def direct_periodogram(freq, template, data, nshifts=50):
    ...
    def chi2_tau(tau):
        return chi2_template_fit(freq, shift_template(template, tau), data)
...
def truncate_template(phase, y, nharmonics):
    fft = np.fft.fft(y[::-1])
    c_n, s_n = zip(*[(p.real/len(phase), p.imag/len(phase))
...
def test_fast_template_method(nharmonics, template, data, samples_per_peak,
                              nyquist_factor):
    t, y, yerr = data
    ...
    freq_template, power_template \
        = model.autopower(samples_per_peak=samples_per_peak,
                          nyquist_factor=nyquist_factor)

    def pdg(freq): return direct_periodogram(freq, temp, data)
...
@pytest.mark.parametrize('samples_per_peak', samples_per_peak_to_test)
@pytest.mark.parametrize('nyquist_factor', nyquist_factors_to_test)
def test_fast_vs_slow(nharmonics, template, data, samples_per_peak,
                      nyquist_factor):
    t, y, yerr = data
    phase, y_phase = template
    w = weights(yerr)
    ...
    model = FastTemplatePeriodogram(template=temp)
    model.fit(t, y, yerr)
    freq_template, power_template_fast \
        = model.autopower(samples_per_peak=samples_per_peak,
                          nyquist_factor=nyquist_factor, fast=True)

    freq_template, power_template_slow \
        = model.autopower(samples_per_peak=samples_per_peak,
...
@pytest.mark.parametrize('samples_per_peak', samples_per_peak_to_test)
@pytest.mark.parametrize('nyquist_factor', nyquist_factors_to_test)
def test_best_model(nharmonics, template, data, samples_per_peak,
                    nyquist_factor):
...
def test_multi_template_method(nharmonics, template, data, samples_per_peak,
                               nyquist_factor, rseed):
...
def test_inject_and_recover(nharmonics, ndata, rseed, period=1.2, tol=1E-2):
    ...
    max_p, best_pars = fit_template(t, y, yerr, template.c_n, template.s_n,
                                    1./period, allow_negative_amplitudes=True)

    signal = TemplateModel(template, parameters=parameters,
                           frequency=1./period)
    model = TemplateModel(template, parameters=best_pars,
                          frequency=1./period)
