...
def direct_periodogram(freq, template, data, nshifts=50):
    ...
    def chi_tau(tau):
        return chi2_template_fit(freq, shift_template(template, tau),
                                 data)
...
def test_fast_template_method(nharmonics, template, data, samples_per_peak,
    ...
    def power(freq):
        return direct_periodogram(freq, temp, data)

    direct_power_template = np.array([power(freq) for freq in freq_template])
...
def test_inject_and_recover(nharmonics, ndata, rseed, period=1.2, tol=1E-2):
    ...
    signal = TemplateModel(
        template, parameters=parameters, frequency=1./period)
