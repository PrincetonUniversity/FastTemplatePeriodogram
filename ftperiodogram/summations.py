...
def _direct_stacked_sums_chunk(t, y_cos_u, w, freqs_chunk, nh):
    ...
    The harmonic phases are generated by a fused recurrence (2026-07-18
    efficiency hunt, skeptic-vetted): one complex exponential on the
    fundamental ``Z = exp(2 pi i f t)`` per (frequency, sample), advanced
    per harmonic by ``Zcur *= Z`` with the ``C``/``S``/``YC``/``YS``
    reductions read off fused matvecs -- the ``(m, 2H, N)`` angle array is
    never materialized, and there is NO recurrence along the frequency
    axis (a frequency cumprod accumulates ~1e-8 drift over a few thousand
    grid steps and was explicitly rejected). Rounding of the harmonic
    recurrence grows only like sqrt(2H) ulp (2H <= 16 here), far below
    the NFFT path's 1e-7 truncation tolerance and at the same ulp class
    as direct cos/sin evaluation.
    """
    fr = np.asarray(freqs_chunk, dtype=float)
    m = fr.shape[0]
    w = np.asarray(w, dtype=float)
    Z = np.exp(2j * np.pi * np.outer(fr, np.asarray(t, dtype=float)))  # (m, N)
    Zcur = Z.copy()
    wc = w.astype(np.complex128)
    uc = np.asarray(y_cos_u, dtype=float).astype(np.complex128)
    C = np.empty((m, 2 * nh))
    S = np.empty((m, 2 * nh))
    YC = np.empty((m, nh))
    YS = np.empty((m, nh))
    # errstate: complex matmul through Accelerate leaves spurious FP
    # status flags set (divide/overflow/invalid) on perfectly finite
    # inputs and results -- verified finite here; genuinely non-finite
    # data still fails loudly downstream (the scan raises on non-finite
    # YM/MM coefficients)
